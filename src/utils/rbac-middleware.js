...
function hasRequiredRole(user, allowedRoles) {
  ...
  if (userGrade && allowedRoles.includes(`${userRole}:${userGrade}`))
    return true;
...
function requireRole(allowedRoles) {
  return (req, res, next) => {
    ...
    if (!req.session || !user) {
      console.warn(
        `[RBAC] Unauthenticated access attempt to protected resource: ${req.path}`,
      );
      return res.redirect(
    ...
    if (!hasRequiredRole(user, allowedRoles)) {
      console.warn(
        `[RBAC] Unauthorized access attempt by ${user.username} (${user.role}) to: ${req.path}`,
      );
...
function requireAPIRole(allowedRoles) {
  return (req, res, next) => {
    ...
    if (!req.session || !user) {
      console.warn(
        `[RBAC API] Unauthenticated API access attempt: ${req.path}`,
      );
    ...
    if (!hasRequiredRole(user, allowedRoles)) {
      console.warn(
        `[RBAC API] Unauthorized API access by ${user.username} (${user.role}) to: ${req.path}`,
      );
...
function applyRBACToRoutes(app) {
  Object.keys(PAGE_ACCESS).forEach((path) => {
    ...
    console.log(
      `[RBAC] Protecting page: ${path} - Allowed: ${allowedRoles.join(", ")}`,
    );
  ...
  Object.keys(API_ACCESS).forEach((path) => {
    ...
    console.log(
      `[RBAC] Protecting API: ${path} - Allowed: ${allowedRoles.join(", ")}`,
    );
