...
async function migrate() {
    ...
    try {
        ...
        const UserRegistration = mongoose.models.UserRegistration || mongoose.model('UserRegistration', new mongoose.Schema({
            firstName: String,
            secondName: String,
            email: String,
            emailHash: String,
        ...
        for (const user of users) {
            const isEncrypted = (val) => typeof val === 'string' && val.split(':').length === 3;
            ...
            if (user.firstName === "[object Object]") {
                console.warn(`[!] Warning: user ${user.username} has corrupted data [object Object]. Skipping to avoid further issues.`);
                errorCount++;
                continue;
            }
