...
const apiLimiter = rateLimit({
    ...
    legacyHeaders: false,
    handler: (req, res) => {
        console.warn(`[RATE LIMIT] API request blocked for IP: ${req.ip} - Path: ${req.path}`);
        res.status(429).json({
            error: 'Too Many Requests',
...
const strictLimiter = rateLimit({
    ...
    message: {
        ...
        message: 'You have exceeded the maximum number of attempts for this operation. Please try again later.',
    },
    standardHeaders: true,
    legacyHeaders: false,
    handler: (req, res) => {
        console.warn(`[RATE LIMIT] Strict limit triggered for IP: ${req.ip} - Path: ${req.path}`);
        res.status(429).json({
            error: 'Too Many Requests',
...
function trackFailedAttempt(ip) {
    ...
    if (record.count >= BAN_THRESHOLD) {
        ...
        console.error(`[SECURITY] IP BANNED for 24 hours: ${ip} - ${record.count} failed attempts`);
    }
}
