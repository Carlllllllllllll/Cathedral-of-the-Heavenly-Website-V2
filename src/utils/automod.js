...
function maliciousFilter(req, res, next) {
  ...
  for (const obj of toCheck) {
    if (isMalicious(obj)) {
      console.error(
        `[AUTOMOD] Malicious request blocked from IP: ${req.ip} - Path: ${req.path}`,
      );
          ...
          embeds: [
            {
              ...
              fields: [
                ...
                { name: "الطريقة", value: req.method, inline: true },
                {
                  ...
                  value: JSON.stringify(obj).substring(0, 500),
                },
              ],
      ...
      return res.status(403).json({
        success: false,
        message:
          "تم حظر طلبك من قبل النظام الأمني. إذا كنت تعتقد أن هذا خطأ، يرجى الاتصال بالإدارة.",
      });
    }
  }
  next();
  ...
  if (now - record.lastAction < COOLDOWN_MS) {
    ...
    if (record.triggerCount >= SPAM_THRESHOLD) {
      return await autoBanUser(
        ...
        res,
        key,
        "حظر تلقائي: محاولات متكررة رغم فترة الانتظار (سبام).",
      );
    }
...
async function autoBanUser(req, res, target, reason) {
  ...
  try {
        ...
        embeds: [
          {
            ...
            fields: [
              ...
              {
                name: "تاريخ انتهاء الحظر",
                value: expiresAt.toLocaleString("ar-EG"),
                inline: true,
              },
              { name: "العنوان الرقمي (IP)", value: req.ip, inline: true },
            ],
    ...
    if (
      ...
      req.session.username.toLowerCase() === target.toLowerCase()
    ) {
      req.session.destroy();
    }
    ...
    return res
      ...
      .json({
        success: false,
        message: "خطأ داخلي في النظام أثناء المراجعة الأمنية.",
      });
  }
}
