...
function createSessionFingerprint(req) {
    ...
    // Normalize IP for fingerprinting to allow for subnet roaming (common in Safari/Mobile)
    let ipPart = ip;
    if (ip.includes('.')) {
...
function validateSessionFingerprint(req, res, next) {
    ...
    if (req.session.fingerprint !== currentFingerprint) {
        console.error(`[SECURITY] Session hijacking detected! Session: ${req.session.id}, User: ${req.session.user?.username}`);
        console.error(`[SECURITY] Expected fingerprint: ${req.session.fingerprint}`);
...
function checkSessionTimeout(req, res, next) {
    ...
    if (req.session && req.session.user && req.session.lastActivity) {
        ...
        if (timeSinceActivity > TIMEOUT) {
            console.warn(`[SECURITY] Session timeout for user: ${req.session.user.username}`);
    ...
    if (req.session && req.session.user) {
        ...
        if (timeSinceRotation > ROTATION_INTERVAL) {
            ...
            req.session.regenerate((err) => {
                ...
                req.session.lastRotation = now;
                console.log(`[SECURITY] Session ID rotated for user: ${req.session.user?.username} (Old: ${oldSessionID}, New: ${req.session.id})`);
                next();
            });
...
function detectSuspiciousActivity(req, res, next) {
    ...
    if (requestsPerMinute > 100) {
        console.error(`[SECURITY] Suspicious activity detected for user: ${req.session.user.username} - ${requestsPerMinute.toFixed(0)} req/min`);
        ...
        return res.status(429).json({
            ...
            message: 'Unusual activity detected. Your session has been terminated for security reasons.',
