...
const API_SECRET = process.env.API_HASH_SECRET || 'church-website-ultra-secret-2024-change-this';
...
const API_ENDPOINTS = {
    ...
    '/api/admin/users/:id/logout-all': hashEndpoint('/api/admin/users/:id/logout-all'),
    '/api/admin/users/:id/password-reset-link': hashEndpoint('/api/admin/users/:id/password-reset-link'),
    '/api/admin/users/:id/password-reset-links': hashEndpoint('/api/admin/users/:id/password-reset-links'),
    ...
    '/api/reset-password/validate/:token': hashEndpoint('/api/reset-password/validate/:token'),
    ...
    '/api/registrations/:id/reactivate': hashEndpoint('/api/registrations/:id/reactivate'),
    '/api/registrations/:id/approve': hashEndpoint('/api/registrations/:id/approve'),
    '/api/registrations/:id/decline': hashEndpoint('/api/registrations/:id/decline'),
    ...
    '/api/admin/gift-shop/purchases': hashEndpoint('/api/admin/gift-shop/purchases'),
    '/api/admin/gift-shop/purchases/:id/accept': hashEndpoint('/api/admin/gift-shop/purchases/:id/accept'),
    '/api/admin/gift-shop/purchases/:id/decline': hashEndpoint('/api/admin/gift-shop/purchases/:id/decline'),
    '/api/admin/gift-shop/purchases/:id/received': hashEndpoint('/api/admin/gift-shop/purchases/:id/received'),
    '/api/admin/gift-shop/items': hashEndpoint('/api/admin/gift-shop/items'),
    '/api/admin/gift-shop/items/:id': hashEndpoint('/api/admin/gift-shop/items/:id'),
    '/api/gift-shop/approvals': hashEndpoint('/api/gift-shop/approvals'),

    // Live Admin
    '/api/admin/live/sessions': hashEndpoint('/api/admin/live/sessions'),
    '/api/admin/live/clear-sessions': hashEndpoint('/api/admin/live/clear-sessions'),
    '/api/admin/live/clear-guests': hashEndpoint('/api/admin/live/clear-guests'),

    // Leaderboard Access
    '/api/admin/leaderboard/access': hashEndpoint('/api/admin/leaderboard/access'),
...
function apiHashMiddleware(req, res, next) {
    ...
    var pathParts = reqPath.split('/').filter(function (p) { return p; });

    if (pathParts.length > 0) {
        // First, check for an exact match of the entire path
        var joinedHash = pathParts.join('/');
        var original = getOriginalEndpoint(joinedHash);

        if (original) {
            req.url = original + (req.url.indexOf('?') !== -1 ? '?' + req.url.split('?')[1] : '');
            req.originalHash = joinedHash;
            return next();
        }

        // Second, check if the first part is a hash for a patterned endpoint
        var hashPart = pathParts[0];
        var patternMatch = getOriginalEndpoint(hashPart);

        if (patternMatch && patternMatch.indexOf(':') !== -1) {
            var patternSegments = patternMatch.split('/').filter(function (p) { return p !== ''; });
            ...
            req.url = reconstructed + (req.url.indexOf('?') !== -1 ? '?' + req.url.split('?')[1] : '');
            req.originalHash = hashPart;
            return next();
        }
    }

    if (reqPath.startsWith('/api/') && !reqPath.startsWith('/api/v1/client-api-map')) {
        if (isMatchingEndpoint(reqPath)) {
            console.warn(`[SECURITY] Blocked unhashed access to protected API: ${reqPath} from IP: ${req.ip}`);
...
function generateClientAPIMap() {
    const clientMap = {};
    Object.keys(API_ENDPOINTS).forEach(original => {
        if (original.startsWith('/api/') || original === '/login' || original === '/logout' || original === '/register') {
