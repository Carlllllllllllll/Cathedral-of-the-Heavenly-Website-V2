...
if (process.env.NODE_ENV === "production") {
  ...
  app.get("/design/:file", (req, res, next) => {
    const file = String(req.params.file || "");
    if (!file.toLowerCase().endsWith(".css")) return next();
    return res.type("text/css").status(404).send("/* Not found */");
  });
} else {
  app.use(
    "/design",
...
class DatabaseBackup {
  ...
  async backupLocalUserRegistrations() {
    ...
    try {
      ...
      if (jsonData.length > this.CHUNK_SIZE * 100) {
        const usersPerChunk = Math.ceil(
          count / Math.ceil(jsonData.length / (this.CHUNK_SIZE * 100)),
        );
      ...
      const statsIndex = this.collectionStats.findIndex(
  ...
  async completeBackupCycle() {
    ...
    this.collectionStats.forEach((stat) => {
      const statusIcon =
